...
## Visualize genes related to pathway(s) of interest
...
p <- pathways[2,1]
visualizeInteractions(obj = object.int, dm = object, show.in = c(4,1), pathway = p)
```

```{r, eval=TRUE, fig.height=7, fig.width=7}
interactionDotPlot(obj = object.int, dm = object, interaction = 4, pathway = p)
```
...
We can be interested in genes participating in pathways with a receptor of interest inside a cluster of interest. Let us say *EGFR* in the first cluster.

```{r, eval=TRUE}
g <- "EGFR"
c <- cluster(object)$names[1]
