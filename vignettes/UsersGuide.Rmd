...
```{r, eval=TRUE}
visualize_interactions(obj = object.int, dm = object)
```

This function will create a *plot* in the R plot window.  

The details of the interactions between two clusters, for example cluster 1 and 2, can also be shown in the plot window with the `visualize_interactions()` function. Note that in the example below we ask for the display of two pairs of cell clusters, pair 1 that contains interactions from cluster 1 to 2, and pair 4 from cluster 2 to 1. (`names(object.inter@LRinter)` returns the cell cluster names in each pair, see function `visualize_interactions()` details.)   

```{r, eval=TRUE}
visualize_interactions(obj = object.int, dm = object,show.in=c(1,4))
```   
...
It is possible to add specific LR interactions to be studied. When using the `cellSignaling()` function, specifying the LR interactions as a dataframe through the `addLR` argument:
...
```{r, eval=TRUE}
object.b <- cellClustering(obj = object, n = 12, method = "simlr", markers = my.markers, classification = "singler")
```

We can compare the results with the one obtained through cell annotation:
```{r, eval=TRUE}
object <- cellClassifying(obj = object, markers = my.markers)
```
...
And then visualized:
```{r, eval=TRUE}
visualize_interactions(obj = object.int, dm = object, show.in = c(4,1))
```

It is possible to look at the genes involved in these specific interactions using the `expression_plot_2()` function.
```{r, eval=TRUE}
expression_plot_2(obj = object, "AREG", "EGFR")
```  

With the function `expression_plot()` it is possible to look at the expression of one gene within each cell.
```{r, eval=TRUE}
expression_plot(obj = object, "ANXA1")
```  
...
```{r, eval=TRUE}
inter.net <- inter_network(obj = object.int, dm = object)
```

We can be interested in genes participating in pathways with a receptor of interest inside a cluster of interest. Let us say *PDE1C* in *cluster 2*.

```{r, eval=TRUE}
g = object.int@LRinter[[1]][1,2]
c = names(object.int@LRinter[[1]][2])
intra.net <- intra_network(dm = object, obj = object.int, goi = g, coi = c)
```
