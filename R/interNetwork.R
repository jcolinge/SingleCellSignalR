...
#' *cell-signaling* folder that contains a compilation of all the 
#' intercellular, ligand-receptor interactions named 
#' 'full-intercellular-network.graphml'. A text and a graphML file in the 
#' *networks* folder containing the intracellularnetwork for each cell cluster 
#' named 'intracell_network_Z.txt' and 'intracell_network_Z.graphml', where Z 
#' is the *c.names* of the cluster.
#'
#' @param obj an object of type SCSRInference
#' @param dm an object of type SCSRDataModel
#' @param clusters a character vector of the clusters names to study
...
#' #if (!is.null(obj.int)) net <- interNetwork(obj.int, obj)

interNetwork <- function(obj, dm, clusters = NULL, pathway = NULL, plot = TRUE,
    ...
    if (!is.null(pathway)) {
        if (!any(grepl(paste0(pathway, collapse = "|"), 
            ...
            stop(paste0(pathway, collapse = ","), "doesn't  
                correspond to anypathway in database. Please check your  
    ...
    if (!is.null(clusters)) {
        if (all(clusters %in% c.names)) {
            ...
            message("Please input clusters names that are described in the  
                object. Analyzing all clusters.")
        }
    }
    ...
    if (!is.null(signal)) {
        for (i in seq_len(length(signal))) {
            ci <- signal[[i]]
            if (names(ci)[1] %in% clusterToStudy & names(ci)[2] %in% 
                clusterToStudy) {
                from <- names(ci)[1]
                to <- names(ci)[2]
                tmp[[i]] <- data.frame(ligand = paste0(from, ".", ci[[1]]),
                    receptor = paste0(to, ".", ci[[2]]), ligand.name = ci[[1]],
                    receptor.name = ci[[2]], origin = from, destination = to,
                    ci[, 3:4], stringsAsFactors = FALSE)
                if (!is.null(pathway))
                    tmp[[i]] <- tmp[[i]][tmp[[i]][, "receptor.name"] %in% 
                        genes.to.plot, ]
                if (nrow(tmp[[i]]) == 0)
                    message("No receptors linked with ", paste0(pathway, 
                        collapse = ","), " in ", to)
            }
        }
        ...
        if (nrow(cellint) == 0) {
            ...
            for (i in seq_len(length(signal))) {
                if (colnames(signal[[i]])[1] %in% clusterToStudy & 
                    ...
                    cell.n <- cellint[cellint$origin == subpop & 
                    cellint$destination == other, ]
                    n.int <- c(n.int, paste(subpop, other, sep = "-"))
                    if (verbose) {
                        message("Doing ", subpop, " and ", other, "...")
                    }
                    ...
                    r.both <- intersect(cell.n$receptor[cell.n$origin == 
                        subpop],
                    cell.n$receptor[cell.n$origin == other])
                    for (r in r.both) {
                        k <- which(cell.n$receptor == r & 
                            cell.n$origin == other)
                        cell.n$receptor[k] <- paste("bis", r)
                    }

                    if (!is.null(dim(cell.n))) {
                        g.cell <- graph_from_data_frame(cell.n, 
                            directed = TRUE)
                        genes <- c(cell.n$ligand, cell.n$receptor)
                        prop.genes <- unique(data.frame(gene = genes, 
                            display.name = gsub("^bis ", "", genes), 
                            gene.type = c(rep("ligand", nrow(cell.n)),
                            rep("receptor", nrow(cell.n))), 
                            expressed.in = c(cell.n$origin, 
                            cell.n$destination), 
                            stringsAsFactors = FALSE))
                        prop.genes <- subset(prop.genes, 
                            !duplicated(prop.genes[[1]]))
                        rownames(prop.genes) <- prop.genes[[1]]
                        g.cell <- g.cell %>%
                        set_vertex_attr(name = "display.name", 
                            value = prop.genes[V(g.cell)$name, 
                                "display.name"]) %>% set_vertex_attr(
                                name = "gene.type", 
                                value = prop.genes[V(g.cell)$name, 
                                    "gene.type"]) %>% set_vertex_attr(
                                    name = "expressed.in", 
                                    value = prop.genes[V(g.cell)$name, 
                                    "expressed.in"])
                        ...
                        if (write) {
                            write.graph(g.cell, 
                                file = paste0("./networks/intercell_network_",
            ...
            # Full network ----------------
            g.cell <- graph_from_data_frame(cellint[, c(1, 2, 7, 8)], 
                directed = TRUE)
            genes <- c(cellint$ligand, cellint$receptor)
            prop.genes <- unique(data.frame(gene = genes, 
                display.name = c(cellint$ligand.name, cellint$receptor.name), 
                gene.type = c(rep("ligand", nrow(cellint)),
                rep("receptor", nrow(cellint))), stringsAsFactors = FALSE))
            prop.genes <- subset(prop.genes, !duplicated(prop.genes[[1]]))
            rownames(prop.genes) <- prop.genes[[1]]
            g.cell <- g.cell %>%
                set_vertex_attr(name = "display.name", 
                value = prop.genes[V(g.cell)$name, "display.name"]) %>%
                set_vertex_attr(name = "gene.type", 
                value = prop.genes[V(g.cell)$name, "gene.type"])
            prop.genes <- unique(data.frame(gene = genes, 
                display.name = c(cellint$ligand.name, cellint$receptor.name), 
                expressed.in = c(cellint$origin, cellint$destination),
                stringsAsFactors = FALSE))
            prop.genes <- subset(prop.genes, !duplicated(prop.genes[[1]]))
            rownames(prop.genes) <- prop.genes[[1]]
            g.cell <- g.cell %>%
                set_vertex_attr(name = "expressed.in", 
                value = prop.genes[V(g.cell)$name, "expressed.in"])
            if (write) {
                write.graph(g.cell, 
                    file = paste0("./networks/
                    full-intercellular-network.graphml"), format = "graphml")
            }
            if (plot) {
                g.plot <- graph_from_data_frame(cellint, directed = FALSE)
                tmp <- do.call(rbind, 
                    strsplit(unique(c(cellint$ligand, cellint$receptor)),
                    split = ".", fixed = TRUE))
                cr <- rainbow(length(clusterToStudy))
                names(cr) <- clusterToStudy
                V(g.plot)$vertex.label <- do.call(rbind, 
                    strsplit(unique(c(cellint$ligand,
                    cellint$receptor)), split = ".", fixed = TRUE))[, 2]
                V(g.plot)$label.color <- "black"
                V(g.plot)$color <- cr[tmp[, 1]]
                V(g.plot)$shape <- c("circle")
                V(g.plot)$shape[unique(c(cellint$ligand, cellint$receptor))
                    %in% cellint$receptor] <- c("square")
                V(g.plot)$size <- 10
                E(g.plot)$width <- cellint$LRscore * 4
                E(g.plot)$color <- "gray30"

                plot(g.plot, vertex.label = V(g.plot)$vertex.label, 
                    main = "Intercellular communication network")

                legend("bottomright", legend = clusterToStudy, fill = cr, 
                    cex = 0.7, xpd = TRUE)
                legend("topright", legend = c("ligand", "receptor"), 
                    pch = c(1, 0), cex = 0.8, xpd = TRUE)
            }
        }
...
simplify_interactions <- function(t, lr = NULL, autocrine = FALSE) {
    ...
    t$type[t$type %in% c("chemical-affects",
        "consumption-controlled-by", "controls-expression-of",
        "controls-phosphorylation-of", "controls-production-of",
        "controls-state-change-of", "controls-transport-of",
        "controls-transport-of-chemical")] <- "control"
    t$type[t$type %in% c("catalysis-precedes",
        "reacts-with", "used-to-produce")] <- "reaction"

    # merge duplicated interactions with same reduced type
    key <- paste(t$a.gn, t$b.gn, t$type, sep = "|")
    dk <- which(duplicated(key))
    for (i in dk) {
        jj <- setdiff(which(t$a.gn == t[i, "a.gn"] &
            t$b.gn == t[i, "b.gn"] & t$type ==
            t[i, "type"]), i)
        for (j in jj) {
            # t[j,'pmid'] <- mergeText(t[j,'pmid'],t[i,'pmid'])
            t[j, "pathway"] <- mergeText(t[j,
                "pathway"], t[i, "pathway"])
            t[j, "detailed.type"] <- mergeText(t[j,
                "detailed.type"], t[i, "detailed.type"])
        }
    }
    if (length(dk) > 0)
        t <- t[-dk, ]
    ...
    to.remove <- NULL
    for (i in rk) {
        j <- setdiff(which(t$a.gn == t[i, "b.gn"] &
            t$b.gn == t[i, "a.gn"]), i)
        if (j < i) {
            # t[j,'pmid'] <- mergeText(t[j,'pmid'],t[i,'pmid'])
            t[j, "pathway"] <- mergeText(t[j,
    ...
    # interactions between 2 ligands or 2 receptors are removed unless
    # 'complex'
    bad <- ((t$a.gn %in% lr$ligand & t$b.gn %in%
        lr$ligand) | (t$a.gn %in% lr$receptor &
        t$b.gn %in% lr$receptor)) & t$type !=
        "complex"
    t <- t[!bad, ]
    ...
    if (!autocrine && !is.null(lr)) {
        # remove LR pairs
        bad <- (t$a.gn %in% lr$ligand & t$b.gn %in%
            lr$receptor) | (t$a.gn %in% lr$receptor &
            t$b.gn %in% lr$ligand)
        t <- t[!bad, ]
    }
