...
#' @description Displays the level of expression of a gene in each cell 
...
#' @param colors 'default' returns the default colorpanel, also accepts 
#' 'rainbow' or 'heat'
...
#' #expressionPlot(obj, 'gene 20')

expressionPlot <- function(obj, name, colors = c("default", "rainbow",
    "heat"), most.variables = TRUE) {
    ...
    if (!is.null(obj@ncounts$matrix.mv) & most.variables) {
        message("Matrix of most variable genes used. To use the whole matrix 
            set most.variables Cj parameter to FALSE.")
        ...
        if (sum(is.na(a)) == 0) {
            ...
            if (sum(as.numeric(data[name, ])) != 0) {
                ...
                plot(x = tsne[, 1], y = tsne[, 2], type = "n",
                    main = list(name, cex = 3, col = "black", font = 3),
                ...
                if (sum(a == 1) != 0) {
                    if (sum(a == 1) == 1) {
                        symbols(x = tsne[a == 1, 1], y = tsne[a ==
                            1, 2], circles = rep(1, 1), inches = 0.05,
                            bg = cr[a[a == 1]], fg = "gray20", add = TRUE)
                    } else {
                        symbols(x = tsne[a == 1, 1], y = tsne[a ==
                            1, 2], circles = rep(1, nrow(tsne[a ==
                            1, ])), inches = 0.05, bg = cr[a[a == 1]],
                            fg = "gray20", add = TRUE)
                    }
                }
                symbols(x = tsne[a != 1, 1], y = tsne[a != 1,
                    2], circles = rep(1, nrow(tsne[a != 1, ])),
                    inches = 0.05, bg = cr[a[a != 1]], fg = "gray20",
                    add = TRUE)
                n <- length(cr)
                Y <- bx[3]
                mY <- (bx[4] + bx[3])/2
                ...
                for (i in seq_len(n)) {
                    polygon(c(x0, x0 + dx, x0 + dx, x0), c(Y +
                        ...
                        Y + i * dy), col = cr[i], border = cr[i],
                        xpd = NA)
                }
                mtext("Expression level", side = 4, at = mY,
                    xpd = NA, line = +5, cex = 1.6)
                mtext("-", side = 4, at = Y, xpd = NA, line = +5,
                    cex = 1.6, adj = 1)
                mtext("+", side = 4, at = bx[4], xpd = NA, line = +5,
                    cex = 1.6)
                par(opar)
            } else {
