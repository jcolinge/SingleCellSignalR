...
#'
#' @return An SCSRInteraction object updated for gene names.
...
#' \dontrun{
#' }
setMethod("resetToInitialOrganism", "SCSRInference", function(obj,
    ...
    for (i in 1:length(LRinter(obj))){
     LRinter(obj)[[i]]$L <- .geneNameConversion(LRinter(obj)[[i]]$L,conversion.dict)
     LRinter(obj)[[i]]$R <- .geneNameConversion(LRinter(obj)[[i]]$R,conversion.dict)        
    }
