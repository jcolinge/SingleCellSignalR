...
#'
#' @details The `pathway` parameter can be set to select one specific   
#' pathway to study.
#' @details `show.in` gives the elements of `signal` to be displayed in the 
#' plot window.
#' @details
#' `write.in` gives the elements of `signal` to be written as pdf files
#' in the *images* folder.
#' @details
#' If `write.out` is TRUE, then the function writes a pdf file with
#' a summary of the all the interactions of `signal` as a chord diagram.
#' @details
...
#'
#' @return The function returns images in the plot window of Rstudio and 
#' images in the pdf format in the *images* folder.
#' @export
...
#' #if (!is.null(obj.int)) visualizeInteractions(obj.int, obj)

visualizeInteractions <- function(obj, dm, pathway = NULL, show.in = NULL,
    write.in = NULL, write.out = FALSE, method = "default", limit = 30) {
    ...
    if (!is.null(pathway) & is.null(show.in) & is.null(write.in)) {
        ...
        if (!any(grepl(paste0(pathway, collapse = "|"), 
            ...
            stop(paste0(pathway, collapse = ", "), " doesn't correspond to any 
                pathway in database. Please check your spelling, use function 
                findPathway().")
        } else {
            genes.to.plot <- unique(c(PwC_ReactomeKEGG[grepl(paste0(pathway,
                collapse = "|"), PwC_ReactomeKEGG$pathway), 1],
                PwC_ReactomeKEGG[grepl(paste0(pathway, collapse = "|"),
                PwC_ReactomeKEGG$pathway), 2]))
            ...
            if (dm@initial.organism != "hsapiens") {
                ...
                genes.to.plot <- merge(genes.to.plot, ortho,
                    by.x = "Hsapiens", order = FALSE)
    ...
    signal <- obj@LRinter
    options(warn = -1)
    if (dir.exists("images") == FALSE & (is.null(write.in) ==
    ...
    cr <- colorRampPalette(c("#FF9900", "#CC0000"))(max(score) -
        min(score) + 1)
    if (write.out == TRUE) {
        pdf(paste("./images/ChordDiagram", method, "all.pdf",
            sep = "_"))
        ...
        circos.trackPlotRegion(track.index = 1, panel.fun = function(x, y) {
            ...
            circos.text(mean(xlim), ylim[1] + 0.1, sector.name,
                facing = "inside", niceFacing = FALSE, adj = c(0.5,
                    0), cex = 1.5)
        ...
        for (i in seq_len(l)) {
            ...
            y <- c(coords[4] + dy * (i - 1), coords[4] + dy *
                (i - 1), coords[4] + dy * i, coords[4] + dy * i)
    ...
    for (n in write.in) {
        if (is.null(dim(signal[[n]])) == TRUE) {
            ...
            if (!is.null(pathway))
                tmp <- tmp[tmp[, 2] %in% genes.to.plot, ]
        }
        if (nrow(tmp) > limit) {
            tmp <- tmp[seq_len(limit), ]
        }
        if (nrow(tmp) == 0 & !is.null(pathway)) {
            message("No receptors linked with ", paste0(pathway,
                collapse = ", "), " in ", cnames[2])
        } else {
            tmp <- tmp[order(as.numeric(tmp[, 4])), ]
            name <- tmp[, 2]
            feature <- tmp[, 1]
            score <- as.numeric(tmp[, 4]) * 100
            cr <- colorRampPalette(c("lightblue", "magenta"))(max(score) -
                min(score) + 1)
            score <- score - min(score) + 1
            dat <- data.frame(name, feature)
            cluster.col <- as.character(c(rep(cols[cnames[2]],
                length(unique(tmp[, 2]))), rep(cols[cnames[1]],
                length(unique(tmp[, 1])))))
            ...
            pdf(paste("./images/", cnames[1], "-", cnames[2],
                "_", method, "_diagram.pdf", sep = ""))
            chordDiagramFromDataFrame(dat, annotationTrack = "grid",
                preAllocateTracks = 1, directional = -1, 
                direction.type = "arrows",
                link.arr.length = link.width, link.arr.width = link.width,
            ...
            circos.trackPlotRegion(track.index = 1, 
                ...
                circos.axis(h = "top", labels = FALSE, minor.ticks = FALSE,
                    major.tick.length = 2, major.at = c(xlim),
            ...
            for (i in seq_len(l)) {
                x <- c(coords[1], coords[2], coords[2], coords[1])
                y <- c(coords[4] + dy * (i - 1), coords[4] +
                    dy * (i - 1), coords[4] + dy * i, coords[4] +
                    dy * i)
                polygon(x, y, col = cr[i], border = cr[i])
            }
            text(coords[1] * 0.925, coords[3] * 0.97, 
                labels = as.character(round(max(as.numeric(tmp[,
                4])) * 100)/100), col = "white")
            text(coords[1] * 0.925, coords[4] * 1.05, 
                labels = as.character(round(min(as.numeric(tmp[,
                4])) * 100)/100), col = "white")
            text((coords[1] + coords[2])/2, coords[3] * 1.03,
                labels = "score")
            legend("topright", fill = rev(unique(cluster.col)),
                legend = cnames[seq_len(2)])
        }

        dev.off()
    }
    if (!is.null(pathway)) {
        message("Ligand-receptor interactions involved in ",
            paste0(pathway, collapse = ", "))
    }
    for (n in show.in) {
        ...
        if (nrow(tmp) == 0 & !is.null(pathway)) {
            message("No receptors linked with ", 
                paste0(pathway, collapse = ", "), " in ", cnames[2])
        } else {
            ...
            circos.trackPlotRegion(track.index = 1, panel.fun = function(x,
                ...
                circos.text(mean(xlim), ylim[1] + 0.1, sector.name,
                    facing = "clockwise", niceFacing = TRUE, adj = c(0,
                    0.5), cex = ifelse(nrow(dat) > 50, yes = 0.5,
                    no = 1))
                circos.axis(h = "top", labels = FALSE, minor.ticks = FALSE,
            ...
            text(coords[1] * 0.925, coords[3] * 0.97, 
                labels = as.character(round(max(as.numeric(tmp[,  4])) * 
                100)/100), col = "white")
            text(coords[1] * 0.925, coords[4] * 1.05, 
                labels = as.character(round(min(as.numeric(tmp[,
                4])) * 100)/100), col = "white")
